...
function cdash_single_business_map() {
	...
	if( is_singular('business') && isset($options['sv_map']) && $options['sv_map'] == "1" ) {
		...
		$contactmeta = $buscontact_metabox->the_meta();
		$locations = $contactmeta['location'];  ?>
		<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDF-0o3jloBzdzSx7rMlevwNSOyvq0G35A&">
		</script>
	...
	if ( $businessquery->have_posts() ) :
		...
		$count = 0;
			while ( $businessquery->have_posts() ) : $businessquery->the_post();
				$business_list .= "<div class='business post_class()'>";
				if($single_link == "yes") {
